...
class ContainerProducts extends Component {
    ...
    modalEdittogle = async (product) => {
        await this.setState({ productEdit: product })
    ...
    render() {
        let productCard = this.state.products.map(product => {
            return (
                <Col sm="4" key={product.id}>
                    <ProductsCard addToCart={(id) => this.addToCart(id)} modalEdittogle={(product) => this.modalEdittogle(product)} products={product} />
                </Col>
            )
        })
        ...
        return (
            <Container className="container" fluid>
                ...
                <Row className="mt-5">
                    <Col sm="9" className="nav justify-conten-center">
                    ...
                    <Col sm="3">
                        <span>Total Price: Rp. {this.state.totalPrice} </span>
                        <Button color="success" size="sm" block>Checkout</Button>
